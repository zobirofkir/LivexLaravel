...
class EarningStats extends BaseWidget
{
    protected function getStats(): array
    {
        ...
        $todayEarnings = Earning::whereDate('created_at', Carbon::today())->sum('amount');
        $monthEarnings = Earning::whereMonth('created_at', Carbon::now()->month)
        ...
        return [
            ...
            Stat::make('Top Earner', $topEarner ? ($topEarner->name ?? "User #{$topEarner->id}") : 'N/A')
                ->description($topEarner ? '$' . number_format($topEarner->total_earnings ?? 0, 2) : 'No earnings yet')
